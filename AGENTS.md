...
## In-flight work (as of 2026-08-14)
...
- ✅ DONE (2026-08-14): perfection plan Phase 0-4 all ticked — registry (P1, `/skills` + `/api/registry/*`), TokenTicker widget (P2, `/widget` + Tauri floating window), OpenClaw usage source (rollup v2 adapter), roadmap experience items (full-text search, health panel, onboarding wizard, read-only demo), closeout (screenshots; legacy repos file-visualizer / ai-workbench / skillhub archived to `~/Documents/05-项目代码/_archive/` on user confirmation). Merged to main via PR #11 (7ed87d8); branch feat/desktop-app was merged & deleted, local branch kept.
- ✅ DONE (2026-08-14): closeout P3 — GitHub archived `Justin-Ju-0413/skillhub` + `ccswitch-usage-widget` (`gh repo archive --yes`); ccswitch-usage-widget moved into `_archive/` (now 4 repos: ai-workbench / ccswitch-usage-widget / file-visualizer / skillhub); AIHome README desktop section + spec status marks (desktop-design 已实现 / sync-design M2 已落地 / Windows 评估); handoff doc created at `docs/superpowers/implementation-notes.md` (plan-referenced `~/Documents/Default Project/` path does not exist); closeout plan `2026-08-10-aihome-closeout-plan.md` ticked 14/14.
- ✅ DONE (2026-09-01): security scan hardening batch (crypto/fetch/migration + test fixture whitelists, scan findings 27→5, remainder adjudicated cross-file call-site FPs) — PR #17.
- ✅ DONE (2026-09-01): vault API manager — `feat/vault-api-manager` (providers + claude-code/codex/opencode adapters + TOOL_ADAPTERS registry, orchestration + 8 API routes, /vault UI, usage attribution override), plan `2026-08-10-vault-api-manager-plan.md` ticked 58/58. Vault file `~/.aihome/vault.enc` (0600)、备份 `~/.aihome/backups/`；e2e 通过 `AIHOME_VAULT_*` 环境变量重定向到 tmp，绝不触碰真实工具配置.
- ✅ DONE (2026-09-03): unified agent tools manager — `feat/agent-tools-manager`（/agents 升级为分区页：默认「AI 工具」检测本机 5 个 AI 工具 claude-code/codex/opencode/Claude Desktop/ChatGPT + 打开动作，「Markdown Agents」分区原样保留；`src/lib/tools.ts` 只读检测 + 60s TTL 缓存 + vault status 只读合并；`/api/tools` + `/api/tools/open` 仅接受目录表内 toolId、固定 `open` 命令；新增 `AIHOME_TOOLS_*` env 重定向），plan `2026-09-03-aihome-agent-tools-manager-plan.md` ticked 23/23 — PR #20.
...
- Desktop lifecycle: SIGTERM/SIGINT cleanup hardened (perfection 1.1, commit 4a03a0e) — normal `kill` exits cleanly; only force-kill (`kill -9`) can still leak the next-server child.
